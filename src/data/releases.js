...
export const RELEASES = [
  ...
  },
  {
    ...
    points: [
      { en: 'All forty card effects redrawn, and shown on a real card before you buy one', fr: 'Les quarante effets de carte redessinés, et montrés sur une vraie carte avant l’achat' },
    ...
    changelog: [
      { en: 'The forty bought treatments are rebuilt. The layer they painted on used to sit above the card’s own title, description, price and tier tag, so every chosen effect covered the words on the card wearing it; the words are now above the treatment everywhere. An effect made of pieces (motes, drops, sparks, stars) is masked off the artwork, measured in the card’s own width so the mask lands on the picture’s real edge on the small face and the blown-up one alike, and the upper tiers use a softer mask that lets light cross the photograph at a third rather than shutting it out', fr: 'Les quarante traitements achetables sont refaits. Le calque sur lequel ils peignaient passait au-dessus du titre, de la description, du prix et de l’étiquette de palier de la carte : chaque effet choisi recouvrait le texte de la carte qui le portait. Le texte passe désormais au-dessus du traitement, partout. Un effet fait de particules (poussières, gouttes, étincelles, étoiles) est masqué hors de l’image, mesuré dans la largeur de la carte pour que le masque tombe sur le vrai bord de la photo aussi bien sur la petite face que sur la grande ; les hauts paliers utilisent un masque plus doux qui laisse la lumière traverser la photo au tiers plutôt que de l’exclure' },
