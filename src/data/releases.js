...
export const RELEASES = [
  ...
  {
    id: 'middle', icon: 'spark', accent: '#34d399',
    title: { en: 'The purse, in the middle', fr: 'La bourse, au milieu' },
    points: [
      { en: 'Your Buckarooz sit in the centre of the bar, where they belong', fr: 'Vos Buckarooz sont au centre de la barre, à leur place' },
      { en: 'Ink is its own button beside the menu', fr: 'L’Encre a son propre bouton à côté du menu' }
    ],
    changelog: [
      { en: 'The purse is centred on the bar now. It never was: Ink shared the chip with it, which made the chip wide, and a wide chip parked on the left drifted further off centre the more money you had. Ink has its own button beside the menu instead, the same size as the bell, and it opens what the number inside the purse used to lead to', fr: 'La bourse est désormais centrée sur la barre. Elle ne l’a jamais été : l’Encre partageait la pastille avec elle, ce qui l’élargissait, et une pastille large calée à gauche dérivait d’autant plus du centre qu’on avait d’argent. L’Encre a désormais son propre bouton à côté du menu, de la taille de la cloche, et il ouvre ce vers quoi menait le nombre dans la bourse' },
      { en: 'The bar is three columns rather than a row, so the purse holds the centre whatever it says and whatever the buttons beside it are doing. Measured at five widths from a 320 pixel phone to a desktop, with a purse from three figures to seven: dead centre at every one', fr: 'La barre est faite de trois colonnes plutôt que d’une rangée : la bourse tient le centre quel que soit son contenu et quoi que fassent les boutons à côté. Mesuré sur cinq largeurs, d’un téléphone de 320 pixels à un ordinateur, avec une bourse de trois à sept chiffres : parfaitement centrée à chaque fois' },
      { en: 'On a very narrow phone five things genuinely do not fit at full size, and something has to give. It is the spacing rather than any of the buttons, which all need a thumb to land on: the purse gives up its padding, the bar gives up its gaps, and the level ring comes down to the size of its neighbours. Without that the purse overlapped the bell by ten pixels at 320 pixels wide', fr: 'Sur un téléphone très étroit, cinq éléments ne tiennent réellement pas en taille normale, et quelque chose doit céder. C’est l’espacement plutôt qu’un bouton, car chacun doit rester atteignable au pouce : la bourse cède ses marges, la barre ses écarts, et l’anneau de niveau descend à la taille de ses voisins. Sans cela, la bourse chevauchait la cloche de dix pixels en 320 pixels de large' },
      { en: 'A pointer keeps the header it had. On a desktop the whole thing is one toolbar at the right, in a line, and centring the purse there would have been wrong: that layout was designed and it stays. The change is the phone\u2019s', fr: 'Un pointeur garde l’en-tête qu’il avait. Sur ordinateur, l’ensemble forme une seule barre d’outils à droite, en ligne, et y centrer la bourse aurait été une erreur : cette disposition a été pensée et elle reste. Le changement concerne le téléphone' }
    ]
  },
  {
    id: 'handover', icon: 'gift', accent: '#38bdf8',
    title: { en: 'Something arrived', fr: 'Vous avez reçu quelque chose' },
    points: [
      { en: 'What the creator sends you now actually arrives, and the game shows you what it was',
        fr: 'Ce que le créateur vous envoie arrive vraiment, et le jeu vous montre ce que c\u2019était' },
      { en: 'Boosters, cards at the tier they were sent at, coins, Ink, levels, effects',
        fr: 'Boosters, cartes au palier envoyé, pièces, Encre, niveaux, effets' }
    ],
    changelog: [
      { en: 'A gift from the creator used to be written straight into your save on the server, and most of it never reached you. A booster was filed under the wrong shape and simply was not in your shelf. A card sent as a Legendary arrived Common, because the tier was written under a name the game does not read and the game falls back to Common when it finds nothing. And anything on your profile - a level, play time, an unlocked effect - lost to your own device: the profile is rewritten every time the game saves your play time, which is immediately before every sync, so the newer copy was always yours and the gift was always thrown away',
        fr: 'Un cadeau du créateur était auparavant écrit directement dans votre sauvegarde sur le serveur, et la plus grande partie ne vous parvenait jamais. Un booster était rangé sous une forme erronée et n\u2019apparaissait tout simplement pas sur votre étagère. Une carte envoyée en Légendaire arrivait en Commune, car le palier était écrit sous un nom que le jeu ne lit pas, et le jeu retombe sur Commune quand il ne trouve rien. Et tout ce qui touche à votre profil - un niveau, un temps de jeu, un effet débloqué - perdait face à votre propre appareil : le profil est réécrit chaque fois que le jeu enregistre votre temps de jeu, c\u2019est-à-dire juste avant chaque synchronisation, la copie la plus récente était donc toujours la vôtre et le cadeau était toujours jeté' },
      { en: 'Nothing is written into your save from outside any more. The creator leaves a note saying what to hand over, and the game puts it away itself, on your device, using the same code that adds a booster you bought or files a card you pulled. There is nothing left to guess and nothing left to lose a race with',
        fr: 'Plus rien n\u2019est écrit dans votre sauvegarde depuis l\u2019extérieur. Le créateur laisse une note indiquant quoi remettre, et le jeu la range lui-même, sur votre appareil, avec le code qui ajoute un booster acheté ou classe une carte tirée. Il n\u2019y a plus rien à deviner ni aucune course à perdre' },
      { en: 'And you are shown what it was. A sheet on launch lists everything that arrived - the pack and how many, the card with its picture and its tier in its own colour, the coins, the Ink - with the creator\u2019s words beside it rather than instead of it. A note that said "here is something from me" and showed nothing was not an answer',
        fr: 'Et on vous montre ce que c\u2019était. Une fiche au lancement liste tout ce qui est arrivé - le paquet et sa quantité, la carte avec son image et son palier dans sa couleur, les pièces, l\u2019Encre - avec les mots du créateur à côté plutôt qu\u2019à la place. Une note disant « voici quelque chose de ma part » sans rien montrer n\u2019était pas une réponse' },
      { en: 'A card sent to you is priced the way the same card pulled out of a booster is priced, from the article\u2019s readership and its tier. A number chosen by hand survived one copy and was recalculated on the next anyway',
        fr: 'Une carte qui vous est envoyée est valorisée comme la même carte tirée d\u2019un booster, d\u2019après l\u2019audience de l\u2019article et son palier. Un montant choisi à la main ne survivait qu\u2019à un exemplaire et était de toute façon recalculé au suivant' },
      { en: 'An announcement now looks like what it is. A gift, a heads-up, an event and a plain note are drawn differently and labelled, instead of all four arriving identical - an outage should not turn up looking like a present',
        fr: 'Une annonce ressemble désormais à ce qu\u2019elle est. Un cadeau, un avertissement, un événement et une simple note sont dessinés différemment et étiquetés, au lieu d\u2019arriver tous les quatre identiques - une panne ne doit pas se présenter comme un cadeau' },
